from processor.utils.util import get_input_file, get_project_path, medir_tempo_execucao, criar_codigo_identificacao, concat_dataframes
...
def process_novo_bolsa():
    ...
    output_file = get_project_path("excel_processor", "output", "Novo_Bolsa_Data_base.xlsx")
    
    processor = NovoBolsaProcessor(str(input_file), str(output_file))
    processor.process()
    print(f" Arquivo '{output_file.stem}' criado com sucesso!")
